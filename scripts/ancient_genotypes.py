# Original downloaded from Josh Schraiber's https://github.com/schraiber/continuity/
# Modified by Malin Pinsky 2026 
# with AI assistance using GitHub Copilot with the GPT-5.3-Codex model.
...
def get_inds_pops(reads_file,ind_file):
	...
	inds_pops = pandas.read_table(ind_file, sep='\\s+', header=None)
	...
	label = [int(np.where(unique_pops == name[0])[0][0]) for name in label_names]
