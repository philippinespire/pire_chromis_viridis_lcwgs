process ANGSD_DIVERSITY {
    ...
    realSFS ${pop}_${era}.saf.idx -P ${task.cpus ?: 8} -fold 1 > ${pop}_${era}.sfs

    # 2. Calculate Pi and Theta using the UNPRUNED SFS
    realSFS saf2theta ${pop}_${era}.saf.idx -sfs ${pop}_${era}.sfs -fold 1 -P ${task.cpus ?: 8} -outname ${pop}_${era}
    ...
    if [ -f "pruned_sites.pos" ]; then
        realSFS ${pop}_${era}.saf.idx -sites pruned_sites.pos -P ${task.cpus ?: 8} -fold 1 > ${pop}_${era}.ld_pruned.sfs
    fi
    """
}